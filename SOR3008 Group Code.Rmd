...
nb <- naive_bayes(Obesity ~ ., data = data_train, laplace = 1)

# View summary of the model
summary(nb)

# Predict the classes on the training data
nb_predictions <- predict(nb, data_train[, -which(names(data_train) == "Obesity")], type = "class")

# Predict the probabilities on the training data
nb_probs <- predict(nb, data_train[, -which(names(data_train) == "Obesity")], type = "prob")

# View the conditional probability tables
# This will show you the tables (distributions) for each feature
print(nb$tables)

# Optional: Check the prior probabilities of the classes
print(nb$prior)

# Check the model performance (if needed)
# For example, calculate accuracy
accuracy <- sum(nb_predictions == data_train$Obesity) / nrow(data_train)
print(paste("Accuracy:", accuracy))
